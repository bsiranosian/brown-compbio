...
#make_kmers(k): returns a list of all the 4^k kmers using an alphabet of 'ATCG'
#Don't need any more... from when I thought we had to make all the possible kmers nodes. 
def make_kmers(k): 
	alphabet = ['A','T','C','G']
	kmer_array = [alphabet]
	for size in range(k-1):
		to_add=[]
		for mer in kmer_array[size]:
			for letter in alphabet:
				to_add.append(mer + letter)
		kmer_array.append(to_add)
	return kmer_array[k-1]
