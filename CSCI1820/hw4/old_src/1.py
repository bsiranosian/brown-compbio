...
def RNAseq(genomefile,readfile, significance, outfile):
	...
	while position < len(sliding_alignments500):
		if in_gene:
			# If less than threshold but not too many bases below it, keep counting
			if sliding_alignments500[position] <= threshold:
				# If too many positions below threshold, stop gene 
	...
	for gene in genes:
		...
		gene.append(1- binom(gene[1]-gene[0],0.031).cdf(gene[2]))
	...
	plt.plot(range(L-500),sliding_alignments500)
	for gene in genes:
		plt.plot([gene[0],gene[1]],[40,40],'k-',lw=5)
	plt.show()
